...
//  g++ -o a test.cpp && ./a
//  g++ -o a test.cpp && a.e
ll row[1500*1500+5][27];
vector<ll> visited(1500*1500+5);
ll next_row=1;
...
{ 
    ...
    {   
        ...
        for(i=0;i<n;i++){
            ll bad=0,location=0;
            for(j=i;j<n;j++){
                if(good_bad[s[j]-'a']=='0') bad++;
                if(bad>k) break;
                if(row[location][s[j]-'a']==0){ // implemented using TRIE DS
                    row[location][s[j]-'a']=next_row++; // probolem link:https://codeforces.com/contest/271/problem/D
                }
                location=row[location][s[j]-'a'];
                if(visited[location]==0) ans++; // count different substring 
                visited[location]=1;
            }
        }
        cout << ans nl;
